...
for i = 2:nof
    ...
    imgname = ['/Users/apple/Desktop/mass_04/fps_20_May_29_2019_rain_01_0001_',...
                        num2str(i-1, '%04d'), '.jpg'];
    % Subtracting the background image is not necessary
    % img = imread(imgname)-imgbg;
    img = rgb2gray(imread(imgname));
    img=imcrop(img,[48 1 412-48 308-1]);
    % figure(1);
    % imshow(img, []);
    ...
    % Intensity to temperature of each centriod  
    iobj=(255-iobj)*(115/255);% 115 is highest temp. in image: 
    % IRB file to image file need to highest temp is 115 C
...
for i = 1:nmax
    ...
    propstemp = regionprops(bw2temp, 'PixelIdxList');
        ...
        for j = 1:numel(propstemp)
            ...
            %constant k/dLv = 
            dropmass{end+1} = trapz(0.0018142*axitemp);
...
% need more vaification: density heat flux method
 rho_h=26*E/(5*10^4);
 %% SWE calculation
 % area of hot plate in m^2
A_hot=size(img,1)*size(img,2)*1.24e-07;
...
% write all information that need to replot
