function [pbp_table, pbp_table_filtered, avi_summary_table, time_series_fbf] = process_one_video( ...
    filename, working_dir, colorbar_image_indexes,...
    final_crop_indexes, ...
    min_thres, minimum_hydro_area, sort_threshold, areaTol, ...
    SWEfactor_threshold, evapTime_min, evapTime_max, dT_min, ...
    mPerPix, m2PerPix2, int_to_temp_conversion, ...
    k_dLv, hf_rho_coeff, rho_water, rho_ice, sigma_ice, mmPerM)
...
tmp = readFrame(vid);
tmpg = im2gray(tmp);
frame_cropped_ref = imcrop(tmpg, final_crop_indexes);
...
[SWE_fbf, time_series_fbf, h_data_cells, hp_area, h_mass_fbf_min, plate_temp] = fbf_method( ...
...
[pbp_table, avi_summary_table, pbp_table_filtered] = append_gap_row_and_summary( ...
    pbp_table, hp_area, rho_water, rho_ice, sigma_ice, h_mass_fbf_min, evapTime_min, evapTime_max, dT_min, plate_temp);
end
